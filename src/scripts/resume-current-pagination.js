import {resumeList} from "./resume-current-array.js";
...
function createCard(templateElementConfig, arrayElement) {
  ...
  if (resumeCurrentSkills.length <= 2) {
  ...
  if (resumeCurrentSkills.length > 2) {
    const visibleSkills = resumeCurrentSkills.slice(0, 2);
    ...
    newElement.classList.add(templateElementConfig.resumeCurrentSkill);
    newElement.classList.add('description_border');
    newElement.textContent = `Еще +${resumeCurrentSkills.length - visibleSkills.length}`
    resumeCurrentTemplateSkills.append(newElement);
  }
